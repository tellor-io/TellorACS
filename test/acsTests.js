...
contract('ACS specific Tests', function(accounts) {
  ...
  let tellorToken;
    ...
    beforeEach('Setup contract for each test', async function () {
        ...
        console.log("tellorToken address", tellorToken.address)
        for(var i = 0;i<10;i++){
          await tellorToken.mint(accounts[i],web3.utils.toWei('200','ether'));
        }
        oracle = await Tellor.new(tellorToken.address);
        console.log("Tellor address", oracle.address)   
        var disputeFee = await oracle.getUintVar(web3.utils.keccak256("disputeFee"))
        console.log("disputeFee", disputeFee)
        var decimals = await oracle.getUintVar(web3.utils.keccak256("decimals"))
